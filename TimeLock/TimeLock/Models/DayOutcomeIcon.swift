...
enum DayOutcomeIcon: String {
    ...
    ///
    /// - 판정 대상은 성공/실패 세션뿐이다 — 긴급·안전 종료(중립)는 성취도, 잘못도 아니다.
    /// - 오늘은 하루가 끝나지 않았으므로 낙관 판정: 1개라도 완주했으면 일단 success,
    ///   자정이 지나 과거가 되면 과거 규칙(전부/혼재/전부실패)으로 확정된다.
    ...
    static func judge(daySessions: [FocusSession], day: Date,
        ...
        let judged = daySessions.filter {
            $0.outcome?.isSuccess == true || $0.outcome?.isFailure == true
        }
        ...
        let hasSuccess = judged.contains { $0.outcome?.isSuccess == true }
        let hasFailure = judged.contains { $0.outcome?.isFailure == true }

        if target == today {
            if hasSuccess { return .success }           // 1개라도 완주 — 일단 성공
